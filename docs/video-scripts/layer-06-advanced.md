...
详细的 Figma MCP 用法见：[docs/examples/recommended-plugins/figma-mcp.md](../examples/recommended-plugins/figma-mcp.md)
...
完整实现见：
- `docs/examples/recommended-plugins/notify-stop.py` — 跨平台桌面通知
- `docs/examples/recommended-plugins/stop-hook.py` — ralph-loop 迭代控制
...
**[Tutorial perspective]** 此功能在实际项目中价值有限，因为很难完全避免文件重叠。但如果项目模块化做得好，这确实是个强大的并行策略。
