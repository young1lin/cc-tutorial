...
### Extended Thinking 工作机制

三个关键词在 Claude Code 内部映射到不同的 thinking budget（思考预算）：

| 关键词 | 触发的思考深度 | 大致 Token 预算 | 适用场景 |
|--------|--------------|----------------|---------|
| `think` | 标准深度思考 | ~10,000 tokens | 常规推理、代码审查、单文件修改 |
| `think harder` | 深层推理 | ~32,000 tokens | 复杂架构决策、多条件 debug、跨文件分析 |
| `ultrathink` | 最大深度推理 | ~128,000 tokens | 极难算法、跨模块并发问题、大规模重构规划 |

**计费说明：**

- thinking tokens 按 **output tokens** 费率计费（Opus: $75/M tokens, Sonnet: $15/M tokens）
