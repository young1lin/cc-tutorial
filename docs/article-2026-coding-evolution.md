...
你明明可以用更好的工具，却选择停留在过去。同样的，AI 辅助编程不是要替代你写代码的能力，而是用更高效的方式解决问题。
...
### 云端 Web 执行（claude.ai/code）

打开 `claude.ai/code`，连接 GitHub，选择仓库，直接下达任务，Claude 会在 Anthropic 的虚拟机里克隆仓库、执行代码、跑测试，最后推一个 branch 给你，可以直接开 PR。

适合这些场景：
- 不在自己电脑旁、用手机或平板发起任务（支持 iOS App）
- 同时跑多个 bug fix，不占本地终端
- 需要 CI/CD 集成的后端修改，让 Claude 写测试再写代码

### 三种在 CLI 和 Web 之间传送的方式

**1. CLI → Web（把任务甩给云端跑）**

在 Claude Code 对话里，消息前加 `&` 前缀，这条消息就会作为新的 Web 会话发到云端执行：
...
用 `/tasks` 查看所有后台 Web 任务的进度。

**2. Web → CLI（把 Web 会话拉回终端继续）**

Web 会话做到一半，想在本地接着干？有三种方法把它"传送"回来：
...
# 方法 C：在 Claude Code 对话里输入
/teleport   # 或简写 /tp
```

也可以在 Web 界面点击 **"Open in CLI"**，复制命令粘贴到终端。

传送时 Claude Code 会自动验证仓库是否匹配、fetch 对应分支、加载完整对话历史，然后你就在本地接上了 Web 会话的上下文。

> **注意**：Session 传送是单向的。CLI 可以拉 Web 会话，但已有的 CLI 会话不能直接推到 Web——用 `&` 前缀可以把当前上下文作为新 Web 会话发出去。

**3. 用 `/tasks` 监控所有后台任务**

```
/tasks      # 列出所有 Web 后台任务
            # 在列表里按 t 可以直接 teleport 进入某个会话
```
...
野卡（WildCard）已于 2025 年关停。目前国内开发者使用较多的是**专门针对 AI 订阅场景开的虚拟卡**，通称"U 卡"，通常以 USDT 充值，开卡即可绑定 claude.ai 完成订阅。
...
部分国内银行发行的**全币种信用卡（Visa/Mastercard）** 配上美国账单地址可以直接扣款成功。关键在于：

- 账单地址填俄勒冈州（Oregon）地址，该州无消费税
- 地址可使用美国地址生成器随机生成一个真实格式的地址
- 成功率因发卡行而异，不是每张卡都能过

这个方式零成本试一下，能过最好，不能过再换其他方案。
...
1. 准备一个美区 Google 账号
2. Google Play 充值余额（可购买美区 Google Play 礼品卡）
3. 手机安装 Claude App（Android）
...
> ⚠️ **封号风险**：Google 账号注册地区、付款 IP 地址、账单地址、实际访问 IP 四者"三跳"（任意三个不一致），很容易触发 Google 风控封账号。建议这四者尽量保持在同一个地区。

### 方案四：iOS App Store（港区 / 美区 Apple ID）
...
> **[Tutorial perspective]** 如果只是个人使用，先试 Pro $20；用量跑满再升 Max。团队 3 人以上，$200 Max + VPS 中转通常比人均 $20 更合算。
...
**阶跃星辰（StepFun）** 也在准备专门的 Coding Plan，出了之后也是一个值得关注的国内选项。目前实测来看，**国内编程最强的两个选择是 GLM-5 和阶跃星辰（开启强制推理模式）**。

> 补充：使用**任何人的邀请链接**注册 GLM Coding，可在当前折扣基础上**再减 10%**，不一定非用上面那条。

**GLM-5 vs Opus 4.6 实测对比**

**[Tutorial perspective]** 以下是个人在实际业务代码上的对比体验（代码涉及业务无法公开，但可以自行复现）：
...
> **[Tutorial perspective]** 结论：GLM Coding 值得作为**入门练手和日常简单任务**的国内替代，但遇到复杂并发、架构级推理等硬核问题，Opus 4.6 的建议质量不在同一档次。预算允许，上真正的 Claude。
...
| **GLM-5** | OpenAI 兼容 | ✅ 服务端拦截 | ✅ VL 模型代理 | ⭐⭐⭐⭐ |
| **Kimi K2.5**（Moonshot） | ✅ **原生 Anthropic 协议** | 待验证 | ✅ 原生多模态 | ⭐⭐⭐⭐ |
| **MiniMax** | OpenAI 兼容 | ⚠️ 仅 CSDN/博客园 | MCP 非原生 | ⭐ |
...
**[Tutorial perspective]** 亲测 9.9 元/月套餐：核心缺陷是 Attention 机制导致**极易遗忘 CLAUDE.md 和 Rules 内容**——项目约定写着写着就丢了，生成的计划和代码质量都差。WebSearch 只能搜 CSDN、博客园等质量极低的站点，图片识别走 MCP 而非原生能力。不限次数，但质量不行。**自己试试，9.9 元，价格不贵。**

**总结：国内优先选 Kimi K2.5（原生 Anthropic 协议）、GLM-5（专项适配）或阶跃星辰（强制推理）。MiniMax 不适合编程场景。**
